# import colorgram
from turtle import Turtle, Screen
import random
turt= Turtle()
my_screen=Screen()
my_screen.colormode(255)

# colors_obj = colorgram.extract('image.jpg', 110)
# colors_list = []
# for i in range(len(colors_obj)):
#     r=colors_obj[i].rgb.r
#     g=colors_obj[i].rgb.g
#     b=colors_obj[i].rgb.b
#     colors_list.append((r,g,b))
# print(colors_list)
# Once we extracted the color list we can save into list we dont need to use in everytime.
color_list = [(213, 154, 96), (52, 107, 132), (179, 77, 31), (202, 142, 31), (115, 155, 171), (124, 79, 99), (122, 175, 156), (229, 236, 239), (226, 198, 131), (242, 247, 244), (192, 87, 108), (11, 50, 64), (55, 38, 19), (45, 168, 126), (47, 127, 123), (200, 121, 143), (168, 21, 29), (228, 92, 77), (244, 162, 160), (38, 32, 35), (2, 25, 24), (78, 147, 171), (170, 23, 18), (19, 79, 90), (101, 126, 158), (235, 166, 171), (177, 204, 185), (49, 62, 84), (164, 203, 208), (183, 190, 204), (83, 70, 40), (11, 112, 106)]

y_position=0

def draw_a_line():
    for i in range(10):
        color=random.choice(color_list)
        turt.dot(20,color)
        turt.penup()
        turt.forward(50)
        turt.pendown()
    

for _ in range(10):
    turt.penup()
    turt.goto(-200,-200+y_position)
    turt.penup()
    draw_a_line()
    y_position+=50
   
    



my_screen.exitonclick()